        ...
        <Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
                ...
                <div class="hidden md:block">
                    ...
                    <Menu as="div" class="relative ml-3">
                        ...
                        <MenuItems class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            ...
                            <a @click="menu_action_handler(item.click)" :href="item.href" :class="[active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700']">{{ item.name }}</a>
                            </MenuItem>
                        </MenuItems>
                        </transition>
                    </Menu>
            ...
            <DisclosurePanel class="md:hidden">
                ...
                <div class="mt-3 space-y-1 px-2">
                    <DisclosureButton v-for="item in userNavigation" :key="item.name" as="a" @click="menu_action_handler(item.click)" :href="item.href" class="block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white">{{ item.name }}</DisclosureButton>
                </div>
                </div>
            </DisclosurePanel>
        </Disclosure>
...
export default {
    ...
    setup() {
        ...
        function menu_action_handler(action)
        {
            switch(action)
            {
                case 'logout':
